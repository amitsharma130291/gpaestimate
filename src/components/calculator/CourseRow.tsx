...
interface CourseRowProps {
  ...
  onChange: (patch: Partial<Omit<Course, "id">>) => void;
  onRemove: () => void;
}
...
export function CourseRow({
  ...
  const creditsErrorId = `credits-error-${course.id}`;
  ...
  return (
    ...
    >
      <div className="grid grid-cols-1 gap-2 border-b border-border px-5 py-2 last:border-b-0 md:grid-cols-[1fr_120px_88px_88px_44px] md:items-center md:gap-4">
          ...
          <div className="flex-1 md:flex-none">
            ...
            <div className="group relative">
              <select
                ...
                className="w-full appearance-none rounded-control border border-border bg-white px-3 py-1.5 pr-7 text-[15px] font-medium text-graphite transition-colors duration-140 hover:bg-mist focus:border-pine-700"
              >
                {scale.grades.map((g) => (
        ...
        <div className="hidden items-center text-[15px] font-semibold tabular-nums text-graphite md:flex">
          {result.qualityPoints !== null ? result.qualityPoints.toFixed(1) : "—"}
        </div>

        <div className="flex items-center justify-between md:justify-center">
          <span className="text-sm text-graphite-muted md:hidden">
            Points: <span className="tabular-nums font-semibold text-graphite">{result.qualityPoints !== null ? result.qualityPoints.toFixed(1) : "—"}</span>
          </span>
          <button
            type="button"
            onClick={onRemove}
...
function ChevronMark() {
  return (
    <svg
      aria-hidden="true"
      viewBox="0 0 20 20"
      className="pointer-events-none absolute right-2 top-1/2 h-4 w-4 -translate-y-1/2 text-graphite-muted transition-transform duration-180 group-focus-within:rotate-180"
    >
      <path
